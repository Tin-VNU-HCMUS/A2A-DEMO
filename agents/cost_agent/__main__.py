...
from agents.cost_agent.agent_executor import CostAgentExecutor  # Giả định có CostAgentExecutor
...
def get_agent_card(host: str, port: int) -> AgentCard:
    return AgentCard(
        name='Cost Agent',
        description='Agent tư vấn chi phí khám chữa bệnh dựa trên triệu chứng và bệnh',
        ...
        skills=[
            AgentSkill(
                ...
                description='Ước tính chi phí khám chữa bệnh, gói xét nghiệm, và dịch vụ y tế dựa trên triệu chứng và bệnh.',
                tags=['chi phí', 'gói dịch vụ', 'bệnh học', 'gói khám', 'gói khám của bệnh', 'xét nghiệm', 'giá khám', 'điều trị', 'chẩn đoán'],
                examples=['Chi phí khám xơ gan là bao nhiêu?', 'Gói khám cho bệnh tăng áp lực tĩnh mạch cửa', 'hãy cho tôi các gói khám của bệnh xơ gan đi', 'Vậy có gói khám xơ gan không?']
            )
        ]
    )
...
def main(
    ...
    async def run_server_async():
        async with app_lifespan(app_context):
            ...
            agent_executor = CostAgentExecutor(mcp_tools=mcp_tools)  # Giả định có CostAgentExecutor
